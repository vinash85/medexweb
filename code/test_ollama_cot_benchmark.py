#!/usr/bin/env python3
"""
Ollama CoT (Chain-of-Thought) Dermoscopy Attribute Benchmark
=============================================================
Uses structured multi-step reasoning prompts to guide VLMs through
dermoscopic feature analysis before arriving at a final label.

Each of the 7 attributes has a step-by-step CoT prompt that forces the
model to describe low-level visual primitives before making a categorical
judgment.  The full reasoning trace is saved alongside the extracted label.

Run inside Docker:
    python3 /home/project/code/test_ollama_cot_benchmark.py
"""
...
RESULTS_CSV = os.path.join(DATA_DIR, "ollama_cot_benchmark_results_round3.csv")
SUMMARY_TXT = os.path.join(DATA_DIR, "ollama_cot_benchmark_summary_round3.txt")
...
# Each query walks the model through structured visual analysis steps
# before requesting a final label choice.
...
# Prompts use simple intuitive labels; this maps them to the original benchmark
# labels used in DermsGemms.csv comparisons.
...
def extract_cot_label(raw, attr_key):
    ...
    Looks for 'LABEL: <value>' anywhere in the output. Falls back to
    keyword matching against the attribute's valid labels.
    """
...
def _keyword_fallback(text, attr_key, valid_labels):
    ...
    Returns CoT-level labels (matching the prompt). The caller maps to
    benchmark labels via map_to_benchmark_label() afterwards.
    """
...
def run_benchmark():
    print("=" * 70)
    print("Ollama CoT Dermoscopy Attribute Benchmark")
    ...
    summary_lines.append("Ollama CoT Dermoscopy Attribute Benchmark — Summary")
    ...
    # CoT quality: % of responses that had an explicit LABEL: line
