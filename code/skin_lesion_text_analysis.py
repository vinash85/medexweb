#!/usr/bin/env python3
"""
Skin Lesion — Text Analysis of MedGemma Attribute Descriptions
===============================================================
Two-level analyses:
  Word-level:
    1. TF-IDF: discriminative terms per Dx class
...
INPUT_DIR = os.path.join(PROJECT_ROOT, "data", "skin_lesion_attributes")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "data", "skin_lesion_text_analysis")

# Also support running from host
if not os.path.exists(INPUT_DIR):
    INPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "skin_lesion_attributes")
    OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "skin_lesion_text_analysis")
...
BOILERPLATE_TOKENS = {
    ...
    # Dermoscopy boilerplate
    "lesion", "observed", "visualized", "identify", "identified",
    "presence", "present", "area", "region", "feature",
}

# ── Classification leak tokens to strip ───────────────────────────────────────
CLASSIFICATION_LEAK_TOKENS = {
    "melanoma", "nevus", "nevi", "naevus", "naevi",
    "carcinoma", "keratosis", "dermatofibroma",
    "malignant", "benign", "diagnosis", "diagnosed",
    "classification", "classified", "classify",
}

# ── Phrase extraction patterns ────────────────────────────────────────────────

# Attribute category headers to strip from phrases (dermoscopy-specific)
ATTRIBUTE_HEADERS_RE = re.compile(
    r"^(?:"
    r"pigment\s+network|globule(?:s)?(?:\s+and\s+dot(?:s)?)?|dot(?:s)?(?:\s+and\s+globule(?:s)?)?|"
    r"streak(?:s)?(?:\s+and\s+pseudopod(?:s)?)?|pseudopod(?:s)?|"
    r"blotch(?:es)?|vascular\s+structure(?:s)?|structureless\s+area(?:s)?|"
    r"blue[- ]white\s+structure(?:s)?|regression\s+structure(?:s)?|"
    r"border\s+characteristic(?:s)?|(?:overall\s+)?symmetry|"
    r"color(?:\s+(?:and\s+)?distribution)?|pigmentation|"
    r"dermoscopic\s+(?:feature|finding)s?"
    r")\s*[:\-–—]\s*",
    re.IGNORECASE,
)
...
PHRASE_PREFIX_PATTERNS = [
    r"^the\s+(?:lesion|image|skin|dermoscopic|border|margin|pigment|color|surface)\s+"
...
def load_data():
    """Load all skin_lesion_attributes JSON files, return DataFrame."""
    records = []
    for fname in sorted(os.listdir(INPUT_DIR)):
        if not fname.startswith("skin_lesion_attributes_") or not fname.endswith(".json"):
            continue
        fpath = os.path.join(INPUT_DIR, fname)
        with open(fpath) as f:
            data = json.load(f)
        dx = data["metadata"]["dx"]
        dx_label = data["metadata"]["dx_label"]
    ...
    print(f"[DATA] Loaded {len(df)} images across {df['class'].nunique()} Dx classes")
    for cls in sorted(df["class"].unique()):
        count = len(df[df["class"] == cls])
        label = DX_LABELS.get(cls, cls)
...
def tfidf_analysis(df, top_n=25):
    ...
    for cls in classes:
        ...
        results[cls] = terms
        label = DX_LABELS.get(cls, cls)
        print(f"\n  Top discriminative terms for [{cls} — {label}]:")
        for t in terms[:10]:
    ...
    for cls, terms in results.items():
        for rank, t in enumerate(terms, 1):
            rows.append({"class": cls, "dx_label": DX_LABELS.get(cls, cls), "rank": rank, **t})
...
def _plot_tfidf_heatmap(tfidf_matrix, feature_names, df, classes, top_n=15):
    ...
    x_labels = [f"{c}\n({DX_LABELS.get(c, c)[:15]})" for c in classes]

    fig, ax = plt.subplots(figsize=(12, max(8, len(all_top_terms) * 0.3)))
    sns.heatmap(
        heat_data, xticklabels=x_labels, yticklabels=all_top_terms,
        cmap="YlOrRd", ax=ax, linewidths=0.5,
    )
    ax.set_title("TF-IDF: Top Discriminative Terms per Dx Class (word-level)")
    ax.set_xlabel("Dx Class")
    plt.tight_layout()
...
def log_odds_dirichlet(df, top_n=25, alpha_prior=0.01):
    ...
    for cls in classes:
        ...
        csv_path = os.path.join(OUTPUT_DIR, f"log_odds_{cls}.csv")
        lo_df.to_csv(csv_path, index=False)

        label = DX_LABELS.get(cls, cls)
        print(f"\n  [{cls} — {label}] — top overrepresented terms (z-score):")
...
def _plot_fightin_words(lo_df, cls, top_n=25, prefix=""):
    """Scatter: z-score vs log10(frequency), label top words/phrases."""
    ...
    dx_label = DX_LABELS.get(cls, cls)
    ax.set_title(f"Fightin' Words ({level}): {cls} ({dx_label}) vs. rest")
    plt.tight_layout()
...
def _keyness_core(df, col, top_n=25, min_freq=3, prefix=""):
    ...
    for cls in classes:
        ...
        csv_path = os.path.join(OUTPUT_DIR, f"{prefix}keyness_{cls}.csv")
        k_df.to_csv(csv_path, index=False)

        label = DX_LABELS.get(cls, cls)
        sig_df = k_df[k_df["significant"] == True]
        print(f"\n  [{cls} — {label}] — {len(sig_df)} significant keywords (BH-corrected p<0.05)")
...
def _plot_volcano(k_df, cls, prefix=""):
    ...
    level = "phrase" if prefix else "word"
    dx_label = DX_LABELS.get(cls, cls)
    ax.set_title(f"Keyness Volcano ({level}): {cls} ({dx_label}) vs. rest")
    ax.legend(loc="upper right", fontsize=8)
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, f"{prefix}keyness_volcano_{cls}.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"  Saved: {path}")
...
def generate_wordclouds(df):
    """Word cloud per Dx class from cleaned token frequencies."""
    print("\n" + "=" * 60)
    print("WORD CLOUDS")
    print("=" * 60)
    ...
    for cls in classes:
        ...
        path = os.path.join(OUTPUT_DIR, f"wordcloud_{cls}.png")
        wc.to_file(path)
        print(f"  Saved: {path}")
...
def summary_report(tfidf_results, logodds_results, keyness_results):
    ...
    lines = [
        "=" * 70,
        "Skin Lesion Text Analysis — Cross-Method Consensus Report (word-level)",
        "=" * 70,
        "",
    ]
    ...
    for cls in classes:
        ...
        print(f"\n  [{cls} — {label}] — {len(consensus)} consensus terms")
...
def phrase_tfidf_analysis(df, top_n=25):
    ...
    for cls in classes:
        ...
        results[cls] = terms
        label = DX_LABELS.get(cls, cls)
        print(f"\n  Top discriminative phrases for [{cls} — {label}]:")
        for t in terms[:10]:
...
def _plot_phrase_tfidf_heatmap(tfidf_matrix, feature_names, df, classes, top_n=10):
    ...
    fig, ax = plt.subplots(figsize=(14, max(8, len(all_top) * 0.4)))
    sns.heatmap(
        heat_data, xticklabels=x_labels, yticklabels=y_labels,
        cmap="YlOrRd", ax=ax, linewidths=0.5,
    )
    ax.set_title("TF-IDF: Top Discriminative Phrases per Dx Class")
    ax.set_xlabel("Dx Class")
...
def phrase_log_odds_dirichlet(df, top_n=25, alpha_prior=0.01):
    ...
    for cls in classes:
        ...
        csv_path = os.path.join(OUTPUT_DIR, f"phrase_log_odds_{cls}.csv")
        lo_df.to_csv(csv_path, index=False)

        label = DX_LABELS.get(cls, cls)
        print(f"\n  [{cls} — {label}] — top overrepresented phrases (z-score):")
        for _, r in lo_df.head(10).iterrows():
...
def generate_phrase_barcharts(df, top_n=20):
    """Horizontal bar chart of top phrases per Dx class."""
    print("\n" + "=" * 60)
    ...
    for cls in classes:
        ...
        label = DX_LABELS.get(cls, cls)
        ax.set_title(f"Top {top_n} Attribute Phrases: {cls} ({label})")
        plt.tight_layout()
        path = os.path.join(OUTPUT_DIR, f"phrase_barchart_{cls}.png")
        fig.savefig(path, dpi=150)
        plt.close(fig)
        print(f"  Saved: {path}")
...
def phrase_summary_report(tfidf_results, logodds_results, keyness_results):
    ...
    lines = [
        "=" * 70,
        "Skin Lesion Text Analysis — Cross-Method Consensus Report (phrase-level)",
        "=" * 70,
        "",
    ]
    ...
    for cls in classes:
        ...
        print(f"\n  [{cls} — {label}] — {len(consensus)} consensus phrases")
        for term, count, methods in consensus[:8]:
...
def main():
    ...
    df = load_data()
    if len(df) == 0:
        print("No data found. Run skin_lesion_attributes.py first to generate attribute descriptions.")
        return

    df = preprocess(df)

    preproc_path = os.path.join(OUTPUT_DIR, "preprocessed.csv")
    df[["image", "class", "dx_label", "clean_text", "n_tokens", "n_phrases"]].to_csv(preproc_path, index=False)
