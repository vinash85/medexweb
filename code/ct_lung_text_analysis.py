#!/usr/bin/env python3
"""
CT Lung Cancer — Text Analysis of MedGemma Attribute Descriptions
=================================================================
Two-level analyses:
  Word-level:
    1. TF-IDF: discriminative terms per cancer type
...
INPUT_DIR = os.path.join(PROJECT_ROOT, "data", "ct_lung_attributes")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "data", "ct_lung_text_analysis")

# Also support running from host
if not os.path.exists(INPUT_DIR):
    INPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "ct_lung_attributes")
    OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "ct_lung_text_analysis")
...
BOILERPLATE_TOKENS = {
    ...
    "one", "single", "slice", "specific", "within", "across",
...
# Attribute category headers to strip from phrases (CT radiology-specific)
ATTRIBUTE_HEADERS_RE = re.compile(
    r"^(?:"
    r"nodule(?:\s+or\s+mass)?\s+characteristic(?:s)?|"
    r"mass\s+characteristic(?:s)?|"
    r"tissue\s+texture(?:\s+and\s+pattern(?:s)?)?|"
    r"surrounding\s+structure(?:s)?|"
    r"(?:other\s+)?radiological\s+sign(?:s)?|"
    r"density|margin(?:s)?|shape|size|"
    r"pleural\s+involvement|lymph\s+node|vascular\s+invasion"
    r")\s*[:\-–—]\s*",
    re.IGNORECASE,
)
...
PHRASE_PREFIX_PATTERNS = [
    r"^the\s+(?:ct|scan|image|nodule|mass|lesion|lung|tissue|tumor|tumour)\s+"
...
def load_data():
    """Load all ct_lung_attributes JSON files, return DataFrame."""
    records = []
    for fname in sorted(os.listdir(INPUT_DIR)):
        if not fname.startswith("ct_lung_attributes_") or not fname.endswith(".json"):
            continue
        fpath = os.path.join(INPUT_DIR, fname)
        with open(fpath) as f:
            data = json.load(f)
        subfolder = data["metadata"]["subfolder"]
    ...
    print(f"[DATA] Loaded {len(df)} images across {df['class'].nunique()} classes")
...
def tokenize_and_clean(text):
    """Lowercase, tokenize, remove stopwords + boilerplate, lemmatize."""
    tokens = word_tokenize(text.lower())
    cleaned = []
    for tok in tokens:
        if not tok.isalpha() or len(tok) < 3:
            continue
        if tok in STOPWORDS or tok in BOILERPLATE_TOKENS:
            continue
        lemma = LEMMATIZER.lemmatize(tok)
        if lemma in STOPWORDS or lemma in BOILERPLATE_TOKENS:
            continue
...
def _plot_tfidf_heatmap(tfidf_matrix, feature_names, df, classes, top_n=15):
    ...
    ax.set_title("TF-IDF: Top Discriminative Terms per Cancer Type (word-level)")
...
def log_odds_dirichlet(df, top_n=25, alpha_prior=0.01):
    ...
    for cls in classes:
        ...
        lo_df = pd.DataFrame({
            "word": words, "log_odds": log_odds, "z_score": z_scores,
            "freq_target": freqs_target, "freq_background": freqs_bg,
...
def _plot_fightin_words(lo_df, cls, top_n=25, prefix=""):
    ...
    for _, r in top_over.iterrows():
        ...
        ax.annotate(label, (r["log_freq"], r["z_score"]),
                     fontsize=7, alpha=0.9, color="firebrick", ha="left", va="bottom")
    ...
    for _, r in top_under.iterrows():
        label = textwrap.shorten(r["word"], width=40, placeholder="…")
        ax.annotate(label, (r["log_freq"], r["z_score"]),
                     fontsize=7, alpha=0.9, color="steelblue", ha="left", va="top")
...
def _keyness_core(df, col, top_n=25, min_freq=3, prefix=""):
    ...
    for cls in classes:
        ...
        for w in vocab:
            ...
            rows.append({
                "word": w, "freq_target": a, "freq_background": b, "freq_total": total_w,
                "freq_target_norm": round(freq_target_norm, 2),
                "freq_bg_norm": round(freq_bg_norm, 2),
                "G2": round(g2, 3), "chi2": round(chi2_val, 3), "p_value": p_val,
                "pct_diff": round(pct_diff, 1), "log_ratio": round(log_ratio, 3),
            })
...
def generate_wordclouds(df):
    """Word clouds weighted by log-odds z-scores (Fightin' Words).

    Only words with positive z-scores (overrepresented in target class)
    are shown, sized by their z-score — not raw frequency.
    """
    print("\n" + "=" * 60)
    print("DIFFERENTIAL WORD CLOUDS (log-odds z-score weighted)")
    print("=" * 60)

    classes = sorted(df["class"].unique())
    alpha_prior = 0.01

    # Build per-class token counts
    global_counts = Counter()
    class_counts = {}
    for cls in classes:
        tokens = []
    ...
    for cls in classes:
        ...
        ).generate_from_frequencies(z_weights)
...
def summary_report(tfidf_results, logodds_results, keyness_results):
    ...
    lines = [
        "=" * 70,
        "CT Lung Text Analysis — Cross-Method Consensus Report (word-level)",
        "=" * 70, "",
    ]
    ...
    for cls in classes:
        ...
        for term in all_terms:
            count = sum([term in tfidf_terms, term in logodds_terms, term in keyness_terms])
            if count >= 2:
                methods = []
                if term in tfidf_terms: methods.append("TF-IDF")
                if term in logodds_terms: methods.append("LogOdds")
                if term in keyness_terms: methods.append("Keyness")
...
def _plot_phrase_tfidf_heatmap(tfidf_matrix, feature_names, df, classes, top_n=10):
    ...
    ax.set_title("TF-IDF: Top Discriminative Phrases per Cancer Type")
    ax.set_xlabel("Cancer Type")
...
def phrase_log_odds_dirichlet(df, top_n=25, alpha_prior=0.01):
    ...
    for cls in classes:
        ...
        lo_df = pd.DataFrame({
            "word": words, "log_odds": log_odds_vals, "z_score": z_scores,
            "freq_target": freqs_target, "freq_background": freqs_bg,
            "freq_total": [freqs_target[i] + freqs_bg[i] for i in range(len(words))],
        })
        lo_df = lo_df.sort_values("z_score", ascending=False).reset_index(drop=True)
...
def generate_phrase_barcharts(df, top_n=20):
    """Horizontal bar chart of top phrases per cancer type."""
    print("\n" + "=" * 60)
...
def phrase_summary_report(tfidf_results, logodds_results, keyness_results):
    ...
    lines = [
        "=" * 70,
        "CT Lung Text Analysis — Cross-Method Consensus Report (phrase-level)",
        "=" * 70, "",
    ]
...
def main():
    ...
    df = load_data()
    if len(df) == 0:
        print("No data found. Check that extraction has completed.")
        return

    df = preprocess(df)

    preproc_path = os.path.join(OUTPUT_DIR, "preprocessed.csv")
    df[["image", "class", "diagnosis", "clean_text", "n_tokens", "n_phrases"]].to_csv(preproc_path, index=False)
